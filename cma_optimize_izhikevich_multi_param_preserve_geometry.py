...
def parse_ascii_wrdata(filepath):
    try:
        ...
        v_n017 = data[:, 1]
        v_n012 = data[:, 3]
        return {"time": t, "v(x1.n017)": v_n017, "v(x1.n012)": v_n012}
...
def score_output(data):
    if data is None:
        return 1e6
    u_vals = np.array(data.get("v(x1.n012)", []))
    v_vals = np.array(data.get("v(x1.n017)", []))
    if len(u_vals) == 0 or len(v_vals) == 0:
        return 1e6
    u_spikes = (u_vals > 0.1).astype(int)
    spike_count_u = np.sum(np.diff(u_spikes) == 1)
    score = abs(np.max(v_vals) - 0.18) + abs(np.min(v_vals)) + abs(spike_count_u - 3) * 100
    return score
