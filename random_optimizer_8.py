...
# Inject a shape/dynamics-based scoring function from Izhikevich differential equations
...
def run_random_search(trials=25):
    ...
    for t in range(trials):
        ...
        try:
            raw_path = os.path.join(RAW_FILE)
            times, u, v = parse_raw_file(raw_path)
            print(f"First few u values: {u[:5]}")
            print(f"First few v values: {v[:5]}")

            score = score_waveform(times, u, v)
        except:
            score = float("inf")
