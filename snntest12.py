...
# ---------------- Dataset: Binary Pattern Parity Classification ----------------
def generate_binary_parity_dataset(n=100, bit_length=8):
    X = torch.randint(0, 2, (n, bit_length))
    Y = X.sum(dim=1) % 2  # 0 = even parity, 1 = odd parity
    return X.float(), Y.long()

# ---------------- Bit Pattern Encoder ----------------
class BitPatternEncoder:
    def __init__(self, T, N, strength=20.0):
        self.T = T
        self.N = N
    ...
    def forward(self, input_series, steps=40):
        ...
        for t in range(steps):
            ...
            for i in range(1, len(self.layers)):
                s = torch.bmm(s.unsqueeze(1), self.weights[i - 1].to(self.device)).squeeze(1)
                s = self.layers[i](s, t, self.coeffs)
                if i == len(self.layers) - 1:
                    spike_accum[t] = s
        return spike_accum

# ---------------- Training Loop ----------------
def train_parity_snn(mode='surrogate'):
    device = 'cuda' if torch.cuda.is_available() else 'cpu'
    B, T = 1, 40
    model = TrainableSNN([8, 12, 2], batch_size=B, mode=mode, device=device)
    encoder = BitPatternEncoder(T, 8)
    ...
    for epoch in range(EPOCHS):
        X, Y = generate_binary_parity_dataset(SAMPLES, bit_length=8)
        correct = 0
        total_loss = 0
        loss_total = 0
...
# ---------------- Run Training ----------------
acc_surrogate = train_parity_snn(mode='surrogate')
acc_sigmoid = train_parity_snn(mode='sigmoid')

plt.plot(acc_surrogate, label='Surrogate Spike')
plt.plot(acc_sigmoid, label='Sigmoid Spike')
plt.xlabel("Epoch")
plt.ylabel("Accuracy (%)")
plt.title("8-bit Parity Task - Surrogate vs. Sigmoid")
plt.grid(True)
plt.legend()
plt.tight_layout()
plt.show()
